...
14. дабавить провоерку статуса ответа
15. баг, дважды написан curFrom
16. вставку в бд можно сделать батчем + транзакция, а не обновлять построчно
17. конфиги для доступа к бд должны лежать в переменных окружения
либо подгружаться из кого то хранилища паролей (vault)
18. подключение и инициализация бд должны быть вынесены
в отдельную сущность
- в идеале какой то singleton для доступа к бд
19. fmt.Println("Connected!") это пишется в логи
20. вставка в бд должна проходить валидацию для защиты от SQL иньекций
